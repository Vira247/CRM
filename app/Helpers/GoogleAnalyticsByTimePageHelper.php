...
class GoogleAnalyticsByTimePageHelper{
	...
	public static function getProduct($sdate,$edate){
		$query = GoogleAnalyticsByTimePage::selectRaw('SUM(page_view) as amount,page_path as name,date')
		->where('page_path','!=','/')
		->whereDate('date','<=',$sdate)
		->whereDate('date','>=',$edate)
		->groupBy('page_path','date')
		->orderByRaw('SUM(page_view) desc')
		->get();
		return $query;
	}
